...
export const METRIC_LABEL_MAP: Record<string, MetricDef> = {
  ...
  rp_ly_hotel:     { label: 'RevPAR LY', isCompare: false, gen: (d, m) => `$${148 + ((d * 5 + m * 2) % 55)}` },
  rp_stly_hotel:   { label: 'RevPAR ST', isCompare: false, gen: (d, m) => `$${142 + ((d * 9 + m) % 50)}` },
  rp_fcst_hotel:   { label: 'RevPAR Fc', isCompare: false, gen: (d, m) => `$${152 + ((d * 11 + m) % 58)}` },
  rp_act_op:       { label: 'RevPAR',    isCompare: true,  gen: (d, m) => `$${120 + ((d * 7 + m * 2) % 45)}` },
  rp_ly_op:        { label: 'RevPAR LY', isCompare: true,  gen: (d, m) => `$${115 + ((d * 5 + m) % 42)}` },
  rp_stly_op:      { label: 'RevPAR ST', isCompare: true,  gen: (d, m) => `$${110 + ((d * 9 + m) % 40)}` },
  rp_fcst_op:      { label: 'RevPAR Fc', isCompare: true,  gen: (d, m) => `$${118 + ((d * 11 + m) % 44)}` },
  // Other Metrics
  oth_pickup_hotel:   { label: 'PU',       isCompare: false, gen: (d, m) => `${2 + ((d * 3 + m) % 8)}` },
  oth_los_hotel:      { label: 'Avg LOS',  isCompare: false, gen: (d, m) => `${2 + ((d * 5 + m) % 5)}.${(d * 3) % 9}` },
  oth_lead_hotel:     { label: 'ALT',      isCompare: false, gen: (d, m) => `${28 + ((d * 7 + m) % 30)}d` },
  oth_adults_hotel:   { label: 'AD',       isCompare: false, gen: (d, m) => `${1 + ((d * 3 + m) % 2)}.${(d * 7) % 9}` },
  oth_children_hotel: { label: 'CHD',      isCompare: false, gen: (d, m) => `0.${((d * 5 + m) % 9)}` },
  oth_guests_hotel:   { label: 'PAX',      isCompare: false, gen: (d, m) => `${380 + ((d * 11 + m) % 80)}` },
  oth_pickup_op:      { label: 'PU',       isCompare: true,  gen: (d, m) => `${1 + ((d * 3 + m) % 6)}` },
  oth_los_op:         { label: 'Avg LOS',  isCompare: true,  gen: (d, m) => `${3 + ((d * 5 + m) % 4)}.${(d * 3) % 9}` },
  oth_lead_op:        { label: 'ALT',      isCompare: true,  gen: (d, m) => `${35 + ((d * 7 + m) % 25)}d` },
  oth_adults_op:      { label: 'AD',       isCompare: true,  gen: (d, m) => `${1 + ((d * 3 + m) % 3)}.${(d * 5) % 9}` },
  oth_children_op:    { label: 'CHD',      isCompare: true,  gen: (d, m) => `0.${((d * 7 + m) % 9)}` },
  oth_guests_op:      { label: 'PAX',      isCompare: true,  gen: (d, m) => `${120 + ((d * 9 + m) % 50)}` },
  // Availability
  av_rooms_hotel:     { label: 'AR',       isCompare: false, gen: (d, m) => `${15 + ((d * 7 + m) % 25)} rm` },
